...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=El+Messiri:wght@400;500;600;700;800&family=Inter:opsz,wght@14..32,300;14..32,400;14..32,600;14..32,700;14..32,800&family=Playfair+Display:ital,wght@0,500;0,600;0,700;1,500&display=swap" rel="stylesheet">
    ...
    <style>
        ...
        * {
        ...
        /* ==============================
           MLA CARD - WITH MARATHI NAMES & IMAGES
        ============================== */
        .mla-card-ref {
            background: #fff;
        ...
        .filter-premium .form-control,
        .filter-premium .form-select {
            border-radius: var(--radius-sm);
            border: 1px solid var(--gray-200);
            padding: 10px 14px;
            font-size: 0.9rem;
        ...
        .filter-premium .form-control:focus,
        .filter-premium .form-select:focus {
            border-color: var(--accent);
            box-shadow: 0 0 0 4px rgba(232, 122, 42, 0.08);
            background: #fff;
        }
        ...
        .badge-accent {
            ...
            border: 1px solid rgba(232, 122, 42, 0.1);
        }
        ...
        .gold-gradient-text {
            background: linear-gradient(135deg, #b37b2e, #b8860b, #d4af37, #e8c97a);
...
<body>
    ...
    <script>
        ...
        const geography = {
            "Thane": ["Thane", "Kopri-Pachpakhadi", "Ovala-Majiwada", "Mira Bhayandar", "Bhiwandi East", "Bhiwandi West",
                "Kalyan West", "Kalyan East", "Dombivli", "Ambernath", "Ulhasnagar", "Mumbra-Kalwa", "Airoli",
                "Belapur"
            ],
            "Nagpur": ["Katol", "Savner", "Hingna", "Umred", "Nagpur South West", "Nagpur South", "Nagpur East",
                "Nagpur Central", "Nagpur West", "Nagpur North", "Kamptee", "Ramtek"
            ],
            "Satara": ["Man", "Karad North", "Karad South", "Patan", "Jaoli", "Wai", "Koregaon", "Phaltan", "Khandala"],
            "Pune": ["Shirur", "Daund", "Indapur", "Baramati", "Purandar", "Bhor", "Maval", "Chinchwad", "Pimpri",
                "Bhosari", "Vadgaon Sheri", "Shivajinagar", "Kothrud", "Khadakwasla", "Parvati", "Hadapsar",
                "Pune Cantonment", "Kasba Peth"
            ],
            "Mumbai": ["Colaba", "Malabar Hill", "Mumbadevi", "Byculla", "Shivadi", "Worli", "Mahim", "Dharavi",
                "Sion Koliwada", "Wadala"
            ],
        ...
        window.showProfile = (id) => {
            ...
            document.getElementById("premiumModalTitle").innerHTML =
                `<i class="bi bi-person-badge me-2"></i>${m.name} · MLA Profile`;
        ...
        const surveyQuestions = [
            ...
            { id: 4, text: "कोणती वचने पूर्ण झाली? (एकाधिक निवडा)", type: "checkbox", name: "q3_promises", options: ["Tree plantation",
                    "Environment protection", "Pothole-free roads", "Beach cleanup"
                ] },
            { id: 5, text: "मतदार म्हणून तुमची गरज विचारली गेली का?", type: "select", name: "q4_need_asked",
            options: ["हो", "अंशतः", "नाही"] },
            ...
            { id: 11, text: "ही कामे भागाच्या गरजांशी जुळतात का?", type: "select", name: "q7_match_needs", options: ["हो",
                    "अंशतः", "नाही"
                ] },
            ...
            { id: 15, text: "निधी वापर पारदर्शक आहे का?", type: "select", name: "q10_transparent", options: ["हो",
                    "अंशतः", "नाही"
                ] },
                ...
                if (q.type === 'stars') {
                    ...
                    for (let i = 1; i <= max; i++) {
                        ...
                        btn.addEventListener('click', function(e) {
                            ...
                            siblings.forEach(s => {
                                if (parseInt(s.dataset.value) <= val) s.classList.add(
                                'active');
                    ...
                    q.options.forEach(opt => {
                        ...
                        cb.addEventListener('change', function() {
                            ...
                            if (this.checked) {
                                if (!formDataStore[q.name].includes(this.value)) formDataStore[q.name]
                                    .push(this.value);
                            } else {
        ...
        function generateReport(mla, stars) {
            ...
            return `
              <div class="report-card">
                ...
                <div class="report-section"><div class="label">📋 Problem Solving Ability</div><div class="value">${getResponse('q1_solve')}</div></div>
                <div class="report-section"><div class="label">🤝 Accessibility to Citizens</div><div class="value">${getResponse('q1_meeting')}</div></div>
                <div class="report-section"><div class="label">💰 Fund Utilization</div><div class="value">${getResponse('q2_fund_works')}</div></div>
                <div class="report-section"><div class="label">✅ Promises Fulfilled</div><div class="value">${getResponse('q3_promises')}</div></div>
                <div class="report-section"><div class="label">📊 Infrastructure Rating</div><div class="value">${getResponse('rating_infra')} / 5</div></div>
                <div class="report-section"><div class="label">🛣️ Roads Rating</div><div class="value">${getResponse('rating_roads')} / 5</div></div>
                <div class="report-section"><div class="label">🧹 Sanitation Rating</div><div class="value">${getResponse('rating_sanitation')} / 5</div></div>
                <div class="report-section"><div class="label">🌳 Environment Rating</div><div class="value">${getResponse('rating_environment')} / 5</div></div>
                <div class="report-section"><div class="label">🏗️ Local Works Done</div><div class="value">${getResponse('q6_local_works')}</div></div>
                <div class="report-section"><div class="label">🎯 Needs Alignment</div><div class="value">${getResponse('q7_match_needs')}</div></div>
                <div class="report-section"><div class="label">💰 Fund Usage Transparency</div><div class="value">${getResponse('q10_transparent')}</div></div>
                <div class="report-section"><div class="label">⭐ Good Work Done</div><div class="value">${getResponse('q12_good_work')}</div></div>
                <div class="report-section"><div class="label">🔧 Areas for Improvement</div><div class="value">${getResponse('q13_improvements')}</div></div>
                <div class="report-section"><div class="label">📝 Additional Comments</div><div class="value">${getResponse('q11_corruption_view')}</div></div>
                <div class="report-section"><div class="label">👤 Voter Name</div><div class="value">${getResponse('optional_name') || 'Anonymous'}</div></div>
